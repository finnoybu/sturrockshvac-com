...
export default function Header() {
  ...
  return (
    <header className="header-calibration text-white relative z-50">
      <div
        className="mx-auto max-w-screen-2xl px-3 sm:px-6 md:px-12 lg:px-8 xl:px-33
                   py-3 md:py-4 flex items-center justify-between gap-2 sm:gap-4"
      >
        {/* Logo */}
        ...
        >
          {NAV_LINKS.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className="text-white hover:text-accent-300 font-semibold transition-colors"
            >
              {link.label}
            </Link>
          ))}
        ...
        <div className="flex items-center gap-2 sm:gap-3 shrink-0">
          <button
            ...
            className="header-hamburger lg:hidden inline-flex items-center justify-center w-10 h-10 rounded-md text-white hover:bg-primary-700 transition-colors"
          >
            {mobileOpen ? (
              <svg
                viewBox="0 0 24 24"
